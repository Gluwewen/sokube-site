...
Activé aujourd'hui sur : `sokube-two-columns`, `sokube-callout` (et l'équivalent direct dans `layouts/_default/expertise.html`), `sokube-features-section`, `sokube-highlight-section`, `sokube-map`, `sokube-client-logos`, `sokube-stats-grid` (couleur liée à sa propre palette, voir `[params.stats]`), `sokube-testimonials`, `sokube-pricing-table`, `sokube-cta` (et `layouts/partials/components/cta.html`/`trainings-extra-sections.html` qui appellent directement `cta-block.html`, sans losanges par défaut), `sokube-carroussel`, `sokube-partners-section`, `sokube-cards-section`, `sokube-top-tips` (et l'équivalent direct dans `layouts/_default/expertise.html`), `faq` (et l'équivalent direct dans `layouts/_default/expertise.html`, couleur liée à sa propre palette, voir `[params.faq]`), `sokube-feature` (couleur liée à sa propre palette, voir `[params.feature]` - répété plusieurs fois par page, généralement à l'intérieur de `sokube-features-section` : chaque bloc peut avoir ses propres losanges). Sur tous les composants sans palette nommée propre, les losanges utilisent toujours la palette "default" (pas de choix de couleur possible sur ces composants-là).

Chaque composant expose les trois mêmes paramètres : `showDiamonds`/`show_diamonds` (`"true"`/`"false"`, défaut `"false"`), `diamondsPosition`/`diamonds_position` (`"top"`/`"bottom"`, défaut `"bottom"`) et `diamondsSide`/`diamonds_side` (`"left"`/`"right"`, défaut `"right"`, comportement historique). Le losange qui touche le bord extérieur (gauche ou droite) est toujours le grand (coloré) ; le petit (sombre) reste du côté du contenu - l'ordre des deux losanges s'inverse donc entre `"left"` et `"right"`, pas seulement leur alignement.

Aspect structurel (règle 3) : la `<section>` appelante doit porter la classe `relative` pour que le positionnement fonctionne (déjà fait sur tous les composants listés ci-dessus). Sur `sokube-map` et `sokube-stats-grid`, qui n'ont pas de `<section>` propre, un `<div class="relative">` dédié a été ajouté. Sur `sokube-client-logos`, la classe `overflow-hidden` a été déplacée de la `<section>` vers `.logo-scroll` (qui la portait déjà en interne pour son défilement) : la garder sur la `<section>` aurait coupé le débordement voulu des losanges.
...
Supporte aussi la décoration "losanges de marque" (voir section dédiée plus haut) : `showDiamonds="true"`, `diamondsPosition="top"/"bottom"` (défaut "bottom") et `diamondsSide="left"/"right"` (défaut "right") ; couleur liée à la palette du bouton ci-dessus.

Pour un saut de ligne sans nouveau paragraphe dans `columnLeft`/`columnRight` (un vrai retour à la ligne dans le YAML est collapsé en simple espace au rendu), utilisez le texte littéral `\n` - `\n\n` démarre un vrai nouveau paragraphe (même convention que sokube-feature/sokube-carroussel/sokube-callout).
...
- `callout` : `title`, `description` (Markdown supporté ; `\n`/`\n\n` pour saut de ligne/paragraphe, voir "Texte sur deux colonnes" ci-dessus), `palette` (optionnel, "default"/"kube"/"hr", défaut "default" - voir `[params.callout]`), `button_text`, `button_link`, `show_diamonds` (optionnel, false par défaut - décoration "losanges de marque", voir section dédiée plus haut), `diamonds_position` (optionnel, "top"/"bottom", défaut "bottom"), `diamonds_side` (optionnel, "left"/"right", défaut "right") — rendu via `sokube-callout`
