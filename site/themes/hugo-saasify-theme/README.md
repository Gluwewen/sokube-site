...
- [🧵 Architecture CSS : thème vs personnalisations NeuroKube](#-architecture-css--thème-vs-personnalisations-neurokube)
...
Le site est accessible sur http://localhost:1414. Toute modification (contenu, templates, assets) est rechargée automatiquement. Pour arrêter : `Ctrl+C`.

⚠️ **Pour que les modifications de CSS (`assets/css/site.css`, `assets/css/neurokube-overrides.css`) soient prises en compte**, utiliser plutôt :

> npm start

Cette commande lance en parallèle le *watch* Tailwind (regénération de `static/css/style.css`) et `hugo server`. Un `hugo server` seul ne recompile pas le CSS.
...
neurokube-site/
├── site
│   ├── assets/         # Sources CSS du site (voir "Architecture CSS" ci-dessous)
│   │   └── css/
│   │       ├── site.css                 # Point d'entrée du build Tailwind
│   │       └── neurokube-overrides.css  # Personnalisations NeuroKube
...
│   │   ├── css         # CSS compilé (style.css, généré, ne pas éditer à la main)
...
- neurokube-card : le shortcode en question qui contient

![Card Shortcode](/docs/images/readme/neurokube-cards.png "Card Shortcode")

##### Configurer son style

Pour une question d'homogénéité dans le site Web, toute la configuration du style des composants **card**, **card-grid** et **cards-section** est centralisée dans le fichier /site/hugo.toml. Si vous utilisez plusieurs shortcodes Hero dans le site, vous aurez donc exactement le même look&feel.
...
[params.cards]
  ...
  [params.cards.default]
    background = "bg-gradient-to-br from-neurokube-100 to-neurokube-50"
    border = "border-neurokube-300"
    hover_border = "hover:border-neurokube-600"
    label = "text-neurokube-600"
    title = "text-neurokube-900"
    text = "text-gray-700"
```
...
### 🧵 Architecture CSS : thème vs personnalisations NeuroKube

Le CSS du site est scindé en deux responsabilités distinctes, pour que les mises à jour du thème `hugo-saasify-theme` n'écrasent **jamais** les personnalisations propres à NeuroKube et n'aient aucun impact non contrôlé sur le projet :

```
site/
├── assets/
│   └── css/
│       ├── site.css                 # Point d'entrée du build Tailwind (voir package.json)
│       └── neurokube-overrides.css  # Personnalisations NeuroKube (cards, boutons...)
└── themes/
    └── hugo-saasify-theme/
        └── assets/css/main.css      # CSS du thème — NE JAMAIS MODIFIER
```

- **`themes/hugo-saasify-theme/assets/css/main.css`** reste strictement identique à la version amont du thème. Il ne doit **jamais** être modifié directement : toute modification y serait perdue (ou source de conflit) lors d'une mise à jour du thème.
- **`assets/css/site.css`** est le point d'entrée du build Tailwind. Il importe d'abord le CSS du thème tel quel, puis les surcharges NeuroKube :
  ```css
  @import "../../themes/hugo-saasify-theme/assets/css/main.css";
  @import "./neurokube-overrides.css";
  ```
  C'est ce fichier qui est référencé par l'option `-i` dans les scripts `start`/`build` de `package.json`, à la place de l'ancien chemin qui pointait directement dans le thème.
- **`assets/css/neurokube-overrides.css`** contient toutes les classes spécifiques à NeuroKube (`.card`, `.card-neurokube`, `.btn-neurokube`, surcharges `.prose .btn-*`, etc.). **Toute nouvelle personnalisation de style doit être ajoutée ici, jamais dans le thème.**
- Tailwind CLI gère nativement les `@import` de fichiers locaux (pas de plugin `postcss-import` supplémentaire nécessaire).
- ⚠️ L'ordre des deux `@import` compte : `neurokube-overrides.css` doit être importé **après** le CSS du thème, car certaines classes (ex. `.card`) y redéfinissent volontairement des styles déjà définis par le thème — l'ordre de chargement détermine quelle définition l'emporte.
